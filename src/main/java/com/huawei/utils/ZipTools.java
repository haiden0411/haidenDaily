...
public class ZipTools
{
    ...
    public static void zipFile(String baseDir, String fileName) throws Exception
    {
        ...
        for (int i = 0; i < fileList.size(); i++)
        {
            ...
            System.out.println("begin to zip file:" + f.getAbsolutePath());
    ...
    public static void upZipFile(String zipFilename) throws Exception
    {
    ...
    public static String getFileNames(String path)
    {
        ...
        for (int i = 0; i < array.length; i++)
        {
            if (array[i].isFile())
            {
                if (array[i].getName().endsWith(".pdf"))
                {  //获得pdf文件名称
    ...
    public static boolean fileExist(String fileNames, String pdfName)
    {
        ...
        if (!"".equals(fileNames))
        {
            ...
            {
                if (pdfName.equals(nameArr[i]))
                {    //如果文件名相同则执行拷贝操作(拷贝到zip目录准备打包)
                    flag = true;
                    break;
                }
            }
